...
​	注册中心：eureka是ap模型，注重可用性，zookeeper是cp，侧向于一致性结果

2-2-2；

​	负载均衡：Ribbon和feign组合使用，在调用过后，需要执行相应的负载均衡策略，选择其中一个实例进行转发

2-2-3；

​	网关：对外接口的聚合，提供认证鉴权/动态路由/流量控制/请求响应处理;权限控制、负载均衡、路由转发、监控、安全控制黑名单和白名单等
