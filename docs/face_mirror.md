...
整体后端架构-业务设计不合理也会成为高并发场景下的绊脚石

和服务器物理资源有关-业务请求量和物理资源配给不匹配-小马拉大鞍

和tomcat、jetty负载服务器有关，外层请求线程相关配置需要跟着物理资源配置，释放物理资源优势

和负载均衡策略有关，已经是高并发场景下，负载均衡的重要性不言而喻，错误的策略会搞崩一台机器，进而瘫痪整个系统

多实例，每个微服务都会有多个实例，用于分担请求压力

和存取介质有关（缓存、磁盘），当然，数据最终需要落在磁盘上保证持久性，但为了更快的响应时间需要缓存加速

和具体业务场景有关，不同业务场景下的请求量不同，在设计高并发接口时，最好隔离好资源
...
现在关系型数据库大都支持事务性，当然，不是事务性，单机数据库的请求处理能力也极其有限。在高并发场景下，仅仅单机就想处理好所有请求数据，问题还是挺大的。



#### 2-2-2、分布式数据库的突围

分布式概念的简单理解就是一个微服务系统部署在多个物理计算机节点上，节点间共同协作完成系统的任务和功能性

为了解开单机数据库的瓶颈限制，现在可以将数据库系统进行分片，每一个实例部署在不同物理节点上，增加并发度。那么，剩下来的就是数据如何落库，对于这个数据库系统来说，它有多个实例分库，所以需要根据负载均衡策略索引到具体分库上，从而实现落库。为了高可用，需要主从结构增加可用性。
...
看并发度，不高时，对系统不产生威胁、响应时间也客观状态下，可直接落。如果直接落库影响到系统整体性能了，就需要考虑异步方式了，可以是mq，也可以是异步调用。



#### 2-2-4、MQ异步平稳落库

对实时性要求不高场景下，可以立即返回，mq消息投递等待被消费，尤其是大数据量情形下，前端返回的特别快，后面最终也会落库。
...
秒杀活动中热点key（数十万用户抢十台iPhone 16场景）-这种是高并发下单纯的争抢，和上面的争抢加其他请求快速复查不一样

这个是典型的高并发请求到同一个key上，不论是单机还是分布式数据库，压力都很大（中小企业常规环境中，特别nb的数据库暂时不谈）。

是否需要一步到位-内存->到数据库，一步处理到位？也不一定

为什么不更新完内存后直接异步mq给消费者平稳消费到数据库中（最终一致性）

这里有个问题，在更新内存时使用分布式缓存时，同样需要预防超卖问题。

库存使用hash分桶，分散在Redis集群中，压力分散在各个节点上，对单个节点的压力就会减少很多（可以提前预热）

换句话说，同时只能有一个人去Redis集群中的某一个主节点上去cas减库存，减完后异步包装个消息，直接返回。减成功或失败都直接在缓存这一层都明了了。（Redis单机10wQPS左右，具体还要看配置，可能多点或者少点。放到集群环境中大家可以估算估算）
...
- 第三代AI范式特征：1750亿参数规模成为新基准，涌现出GPT-4、Claude等通用模型
- 开源生态崛起：LLaMA-3、Falcon等可商用模型推动技术民主化
- 工程化挑战：千卡集群训练、推理加速等关键技术突破
...
阶段关键任务技术指标POC验证场景选择与数据准备准确率>85%方案设计架构选型与安全评审响应时间<2s生产部署灰度发布与A/B测试并发量>1000QPS
